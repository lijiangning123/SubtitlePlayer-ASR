...
(function() {
  ...
  DP.applySummaryProviderDefaults = function applySummaryProviderDefaults(force) {
    ...
    DP.summaryApiKey.placeholder = saved.apiKeyMask ? `已保存：${saved.apiKeyMask}；留空保留原 Key` : '粘贴 API Key；留空表示保留已保存的 Key';
    DP.summaryConfigHelp.textContent = defaults.help;
    const active = DP.summaryConfigCache?.provider || 'openai';
    const stateText = saved.apiKeySet ? `已保存 API Key：${saved.apiKeyMask || '******'}。留空可保留原 Key。` : '尚未保存 API Key。';
